...
| **WIN-21** | **Purpose-built dialogs, retiring `text_prompt`.** *Progress:* a generic typed `form_dialog()` (text / password / checkbox / choice, N fields) now backs new channel, DND, display name, password and the one-time-secret view. *Remaining:* new DM and create-webhook still call `text_prompt`; convert those two and delete the helper. | P1 | S |
