...
| **WIN-17** | **Inline image thumbnails (REQ-142).** **PARTIAL — do not treat as done.** **Landed and verified:** uploads declare a **real mime** from the extension (every upload was `application/octet-stream`, so nothing downstream could tell an image from a zip — REQ-142 was impossible regardless of client quality) and a **correct basename** (splitting only on `/` meant a Windows upload declared its whole path as the filename). Also landed: an in-memory attachment fetch in the core (`oc_client_fetch_attachment` → `OC_EV_ATTACHMENT_DATA`, bounded, no temp file per ARCH-88), WIC decode-from-memory, and layout that reserves the box so the transcript does not jump when a bitmap decodes. **A thumbnail has been seen rendering correctly on Windows, so the pipeline works end to end.** *Remaining defect:* the bitmap cache misses on subsequent frames, so the image reverts to the placeholder and is re-fetched in a loop (`n_thumbs` pegged at the 32-entry cap with only two distinct attachments). The first cause was comparing render-target pointers to decide ownership — replaced with an explicit flag, which made it render — but a second lookup/eviction bug remains. Next step: log the ids actually held in `g_thumbs` at draw time versus the id looked up. Click-to-expand not started. | P1 | M |
