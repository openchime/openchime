...
| **WIN-17** | **Inline image thumbnails (REQ-142).** **PARTIAL — do not treat as done.** Landed and verified: uploads now declare a **real mime** from the extension (every upload was `application/octet-stream`, so nothing downstream could tell an image from a zip — REQ-142 was impossible regardless of client quality) and a **correct basename** (splitting only on `/` meant a Windows upload declared its whole path as the filename). Also landed: an in-memory attachment fetch in the core (`oc_client_fetch_attachment` → `OC_EV_ATTACHMENT_DATA`, no temp file, per ARCH-88), WIC decode-from-memory, an RT-scoped bitmap cache, and reserved layout so the transcript does not jump when a bitmap decodes. **Not working: the fetch never completes, so no thumbnail ever renders** — the placeholder (which still shows the filename) is what you get. Unresolved: no error surfaces, and the daemon-side DOWNLOAD_BEGIN path looks correct on inspection; the next step is to instrument whether the DOWNLOAD_BEGIN is written and whether DOWNLOAD_END is reached with `fp == NULL`. Click-to-expand not started. | P1 | M |
