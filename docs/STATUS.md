...
| 214 surface storage usage to admins | 🟡 | Free space is sampled (`statvfs`) each pass and logged with every reclamation, and the policy is logged at boot. **The client-facing surface is not built** — no wire message carries usage to owners/admins yet. |
| 215 automatic oldest-first eviction | 🟡 | **Built**: oldest-first under pressure, default-on, disable with `OPENCHIME_EVICT=off`, grace window via `OPENCHIME_EVICT_GRACE_HOURS` (default 24), tombstone keeps the row so the message stays readable, and `DOWNLOAD_BEGIN` returns the distinct `OC_ERR_ATTACHMENT_GONE`. Headless-tested. **Not done:** the audit-log entry per eviction (REQ-251 is itself unbuilt), and no client renders the tombstone yet. |
