...
| 139 activity feed / notification inbox | ✅ 🔵 | **Built (ARCH-95):** a union of three queries — mentions of you, reactions to your messages, replies under your threads — excluding your own actions and gated on current membership. No maintained table: the rows were already stored and indexed. `users.activity_seen_ms` is a watermark for "what is new", deliberately not per-item read state. Win32 fills the **Activity** rail view; clicking an item jumps to the message. **TUI: none.** |
