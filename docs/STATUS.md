...
| REQ | Status | Notes |
|-----|--------|-------|
| 201 no database engine in any client | 🔵 ✅ | **Built** (ARCH-88). No client links SQLite: the session token **and** the TOFU pin live in the OS credential store, and the cached history, offline outbox and workspace book are plain files under the state directory (an append-only CRC'd record log, folded on load and compacted; whole-file atomic rewrites for the small ones). `third_party/sqlite`, `-lsqlite3`, the borrowed `daemon/migrate.c` and the five client migrations are gone from every client target — the Win32 binary went 2.9 MB → **1.9 MB**. Verified on Windows end to end: sign-in, send, restart, cached history and silent reconnect, with `strings` showing no SQLite in any client binary. No migration off the old `state.db`, which the frontends delete on first run (one re-sign-in, one re-TOFU). |
