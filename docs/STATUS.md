...
| 228 drafts, scheduled & sent | ✅ 🔵 | **Built 2026-08-01.** A row at the top of the **Home sidebar** — where Slack keeps it, not a rail destination — carrying a count of drafts plus anything that failed to send. Opens a pane with three tabs: Drafts, Scheduled (REQ-224) and **Sent**, which needed no server work at all: REQ-104's search already answers `from:me` filters-only, access-scoped, newest first. Empty states carry Slack's copy and a New Message button. **TUI: none.** |
