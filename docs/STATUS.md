...
| REQ | Status | Notes |
|-----|--------|-------|
| 201 no database engine in any client | 🔵 🔨 | **Decided (ARCH-88), not yet built.** The session token already lives only in the OS credential store (WIN-56). Still to move: the TOFU pin and workspace book into the credential store, and the cached history + outbox into plain files — after which `client/core/store.c`, `third_party/sqlite`, `-lsqlite3` and the borrowed `daemon/migrate.c` leave every client target. **Today all four still sit in SQLite**, so this row is the honest gap, not a done deal. |
