...
| 220 rich text | ✅ 🔵 | **Built 2026-07-31 (WIN-90, ARCH-100, [MARKDOWN.md](./MARKDOWN.md)):** `client/core/richtext.c` parses the dialect — Slack-compatible inline emphasis (`*bold*`, `_italic_`, `~strike~`, backticks, `>`), extended with the ordered/unordered lists Slack's markup has no syntax for — and returns spans over the unchanged plain-UTF-8 body, so search and the mention byte offsets keep addressing the same text. In `client/core/` and never the daemon: formatting needs no server knowledge, unlike mentions (ARCH-89). Ambiguity is where the work is, and it is pinned down by `tests/test_richtext.c`: `2 * 3 * 4` is arithmetic, `a_variable_name` is an identifier, `snake__case__here` is one too, an unclosed delimiter never restyles the rest of the message, code suppresses everything inside it, and `\*` writes a literal asterisk. Win32 renders it as DirectWrite ranges beside the existing mention highlighting, with the delimiters *removed* in both the transcript and the composer, so a body is drawn the same way wherever it appears. Slack's `<URL\|label>` links and HTML-entity escaping are deliberately **not** adopted: both are API-layer artifacts that would put `&amp;` in the FTS5 index. **Either editor, by preference** (WIN-103, Preferences → Messages → Message input): *Rich text* formats as you type, *Plain text* shows the markup and never restyles it. Rich is the default. Plain text collapses the formatting toolbar with it (WIN-104) — buttons for markup already on screen, and its row would otherwise be an empty strip. It is one flag rather than a second editor — plain text is simply "nothing is hidden", which makes every rule below inert. **The rich editor is WYSIWYG** (WIN-101): it shows formatting and never markup, exactly as the transcript does — the delimiters stay in the buffer, drawn with no ink and no width, and the caret canonicalises past them so no keypress lands in a character that is not there. Markup can never *appear* without being typed: an edit that stops a construct parsing deletes its delimiters rather than revealing them. **Authoring** (WIN-96): a formatting toolbar over the composer and Slack's chords wrap the selection in those same delimiters — never a rich-text model, because ARCH-100 §5 requires everything the toolbar produces to be expressible in text. It toggles off on a second press, trims the selection to its non-space core (a delimiter beside a space is not a delimiter), and marks every line a selection touches for the block forms, numbering `1. 2. 3.`. **The TUI does not render or author it yet** — the parser is shared and waiting for it. |
...
| 223 drafts | ✅ 🔵 | **Built 2026-08-01 (WIN-91, ARCH-101, migration 0030).** Win32: a draft is written ~2s after you stop typing and immediately on leaving the conversation, losing focus or quitting, and **only when it changed** — a device that merely visits a conversation must not write back the copy it holds over a newer one. Restored on entering; an incoming draft from another device fills the field only when it is empty and untouched, never over live typing. A pencil marks the conversation in the sidebar (not counted as unread), and a **Drafts** rail destination exists only while you have one, as Slack's "Drafts & sent" does. **TUI: none.** Server half: `drafts` keyed `(user_id, channel_id, thread_root)` with `SET_DRAFT`/`LIST_DRAFTS` (0x00C0/0x00C1) and `DRAFT`/`DRAFTS` back; an empty body is the delete, membership is required, over-long bodies truncate. `DRAFT` fans to the writer's **other** connections only. Cleared on send inside the send's own transaction — server-side, so a client that dies before its own cleanup does not leave sent text in every other device's composer — and with the user on removal, which also sweeps drafts for the DM channels that die with them. **Decided 2026-07-31 (ARCH-101).** Server-stored in a `drafts` table keyed `(user_id, channel_id, thread_root)` with its own ops — deliberately not `client_settings`, which is partitioned per frontend and would leave a GUI draft invisible in the TUI. `thread_root` is in the key from day one (0 = the channel) so thread drafts cost a client change later, not a migration. Last-writer-wins, made survivable by two client rules: only write a draft you changed, and never overwrite a composer being typed in. Kept on archive and on leaving (both reversible); deleted on send, channel deletion and user removal. Sidebar marks a conversation holding one, matching Slack; not counted as unread. In scope for compliance capture (REQ-276); DLP still redacts at send (REQ-277). |
| 228 drafts, scheduled & sent | ✅ 🔵 | **Built 2026-08-01.** A row at the top of the **Home sidebar** — where Slack keeps it, not a rail destination — carrying a count of drafts plus anything that failed to send. Scheduling is reached from a chevron beside Send in the composer — *In 30 minutes / In 1 hour / Tomorrow 9:00*. Opens a pane with three tabs: Drafts, Scheduled (REQ-224) and **Sent**, which needed no server work at all: REQ-104's search already answers `from:me` filters-only, access-scoped, newest first. Empty states carry Slack's copy and a New Message button. **TUI: none.** |
| 229 new message pane | ✅ 🔵 | **Built 2026-08-01.** A pane in the main area rather than a modal: a `To:` type-ahead over **people and channels together** (`oc_complete_targets`, shared core), chips you can remove, the ordinary composer with its formatting toolbar, and autosave to an **unaddressed draft** so closing it loses nothing. Send resolves the recipients into a channel, a DM or a group DM. Email-address invites are deliberately out — there is no mail path (REQ-033 mints tokens). **TUI: none.** |
