...
| 220 rich text | ⛔ | **Dialect settled 2026-07-31 (ARCH-100, [MARKDOWN.md](./MARKDOWN.md)); not yet built.** Slack-compatible inline emphasis (`*bold*`, `_italic_`, `~strike~`, backticks, `>`), extended with the ordered/unordered lists Slack's markup has no syntax for. Parsed in `client/core/`, never the daemon — formatting needs no server knowledge, unlike mentions (ARCH-89) — and returns spans over the unchanged plain-UTF-8 body, so search and the mention byte offsets keep addressing the same text. Slack's `<URL\|label>` links and HTML-entity escaping are deliberately **not** adopted: both are API-layer artifacts that would put `&amp;` in the FTS5 index. |
