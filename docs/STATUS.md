...
| 212 messaging survives storage exhaustion | ⛔ | Specified (ARCH-77). No disk-space awareness in the daemon today. |
| 213 reclaim orphaned/aborted blobs | ⛔ | Specified (ARCH-77). **`oc_blob_delete` currently has no caller** — deleting a message does not reclaim its attachment bytes, so blob storage grows monotonically. Cheapest fix and the first tier to build. |
