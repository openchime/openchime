...
| 131 recurring daily DND window | ✅ | `SET_DND` stores a daily UTC minutes-of-day window on `users` (wraps past midnight); governs push, not in-app unread (ARCH-72). **This is one of three mechanisms, not all of DND** — the transient "pause until 17:00" is REQ-278 (⛔, not expressible in this model), a per-weekday schedule is REQ-136 (⛔), a workspace default is REQ-279 (⛔). |
...
| 136 recurring notification schedule | ⛔ | Single daily window built (REQ-131); the per-weekday form (Slack's Every day / Weekdays / Custom, independent start+end per day) is unbuilt. Open question recorded: a per-weekday window is only meaningful against a *local* calendar day, so this likely needs the timezone from REQ-241. |
| 278 pause notifications until an instant | ⛔ | **Scoped 2026-07-31, and the gap the DND rows hid.** "Do not disturb until 17:00" / for 30 minutes is a **one-shot absolute instant** and cannot be represented by REQ-131's minutes-of-day pair — storing it there would silence that time *every* day. Needs `dnd_until_ms`, presets + custom, resume-now, and the state carried to other users (REQ-122). The shape is already proven in-tree by custom-status expiry: an absolute stamp the daemon enforces **on read**, so no reader needs a clock and nothing sweeps. Open: whether a sender may override a pause (Slack allows once/day), which weakens the guarantee. |
...
| 287 mentioning a non-member is never silent | ⛔ | **Scoped 2026-07-31.** Resolution needs channel membership (ARCH-89) but the highlight is *syntactic*, so a mention that notified nobody renders identically to one that worked — accent-coloured and apparently fine. The defect is the false confirmation, not the missing invite. Wants a private notice to the sender offering add / send-them-the-message / nothing, the private-channel **history disclosure** named rather than one-clicked (cf. REQ-036a), and the dead name rendered as plain text so the transcript stops asserting something untrue. |
| 288 public-channel mention notifies non-members | ⛔ | **Scoped 2026-07-31, needs a product decision.** Slack's documented split: in a public channel a mentioned non-member *does* get an Activity-feed notification; in a private channel they get nothing and cannot see the message. Ours does neither — `store_mentions` joins `channel_members` for every kind, which is right for private and too wide for public. Changes who receives notifications, so it is recorded rather than assumed; the daemon change is confined to that one membership test. |
