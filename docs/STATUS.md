...
- ~~**WIN-90 / REQ-220** — rich text.~~ **Built 2026-07-31 (ARCH-100).** The
  parser is `client/core/richtext.c` — shared, so the TUI inherits it when it
  catches up — and Win32 renders its spans as DirectWrite ranges. The toolbar
  and its shortcuts are WIN-96, deliberately separate: they are a second way to
  author what you can now already type.
