...
- ~~**Rich text / formatting toolbar**~~ (REQ-220) — **built 2026-07-31** (WIN-90 then WIN-96, ARCH-100): the dialect parses in `client/core/richtext.c`, Win32 renders it as DirectWrite ranges (delimiters removed in the transcript, faint in the composer), and the toolbar plus Slack's chords author it. No longer blocks snippets (REQ-226). Three defects surfaced around it, none on any list: composer **Ctrl+C copied nothing** (WIN-98), search left open floated its native EDIT over the **DM picker** (WIN-99), and **double-click selected no word anywhere** (WIN-100) — lost when WIN-80 replaced the native EDIT, which had done it for free.
...
- **Rich text** (REQ-220) — new to this list on 2026-07-31, because the parser now exists: `client/core/richtext.c` is shared and the GUI already renders its spans, so the TUI's half is mapping the same spans to tuikit attributes (MARKDOWN.md §6). Until it does, a `*bold*` message shows its asterisks — legible, which the dialect guarantees, but the formatting is the GUI's alone. Authoring is the GUI's alone too (WIN-96). **P1**
