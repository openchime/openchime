...
static void test_embedded_schema(void) {
    ...
    CHECK(err == NULL);
    CHECK(oc_schema_version(db) == 38);   /* + reactions/threads/FTS/cursors/identity/attachments/webhooks/notify/client_settings/enrollment/mute/drafts/scheduled/snooze/schedule/keywords/threads/upload-idempotency */
